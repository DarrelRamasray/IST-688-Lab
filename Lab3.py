...
st.sidebar.caption("Configure Chatbot")
buffer_mode = st.sidebar.radio("Buffer Type:", ("Last 2 user turns", "Token limit"), index=0,)
...
if "client" not in st.session_state: #Build the client once per session, not on every rerun
    try:
        api_key = st.secrets["OPENAI_API_KEY"] #Key comes from Streamlit secrets
    except Exception:
...
@st.cache_resource
def get_encoding(): #Load the tokenizer once and reuse
    if tiktoken is None:
        return None
    try:
...
def buffer_by_user_turns(messages, turns=buffer): #Part B, Step 3: last 2 user messages and the replies to them
    ...
    start = user_indexes[-turns] #Slice from the 2nd-to-last user message
...
if prompt := st.chat_input("What is up?"):
    ...
    messages_to_send = [system_msg] + messages_to_send #Prepended AFTER buffering
    ...
    try:
        stream = client.chat.completions.create(
            model = model_to_use,
            messages=messages_to_send, #The buffered list, not full history
            stream=True,
        )
...
if "last_request_tokens" in st.session_state: #Part B, Step 4a shown in the running app, not just calculated
    st.sidebar.caption(
